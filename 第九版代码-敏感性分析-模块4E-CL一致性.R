...
## ############################################################################
## E.2 ICC：CL_ind 的跨模型一致性（含 95% CI）
## ############################################################################

message("\n--- E.2 ICC：CL_ind 的跨模型一致性（含 95% CI）---")

## 检查 psych 和 lme4 是否可用
HAS_PSYCH <- requireNamespace("psych", quietly = TRUE)
HAS_LME4  <- requireNamespace("lme4",  quietly = TRUE)

if (HAS_PSYCH && HAS_LME4) {
  
  ## 使用 psych::ICC() 计算完整 ICC（含 CI）
  library(psych)
  icc_CL_result <- ICC(CL_ind_data[, -1], alpha = 0.05)  # 95% CI
  
  ## 提取 ICC(2,1) 和 ICC(2,k)
  ## psych::ICC 输出的 results 行顺序：
  ##   1 = ICC1, 2 = ICC2 (即 ICC(2,1)), 3 = ICC3
  ##   4 = ICC1k, 5 = ICC2k (即 ICC(2,k)), 6 = ICC3k
  
  icc_CL_2_1     <- icc_CL_result$results$ICC[2]
  icc_CL_2_1_lo  <- icc_CL_result$results$`lower bound`[2]
  icc_CL_2_1_hi  <- icc_CL_result$results$`upper bound`[2]
  
  icc_CL_2_k     <- icc_CL_result$results$ICC[5]
  icc_CL_2_k_lo  <- icc_CL_result$results$`lower bound`[5]
  icc_CL_2_k_hi  <- icc_CL_result$results$`upper bound`[5]
  
  icc_CL_method  <- "psych::ICC (F-test based CI)"
  
  cat("\n=== CL_ind ICC 结果（psych 包，含 95% CI）===\n")
  cat(sprintf("  ICC(2,1) = %.4f [95%% CI: %.4f - %.4f]\n", 
              icc_CL_2_1, icc_CL_2_1_lo, icc_CL_2_1_hi))
  cat(sprintf("  ICC(2,k) = %.4f [95%% CI: %.4f - %.4f]\n", 
              icc_CL_2_k, icc_CL_2_k_lo, icc_CL_2_k_hi))
  
} else {
  
  ## 备用方案：简化 ICC + Bootstrap CI
  message("⚠️ psych/lme4 包未安装，使用简化方法 + Bootstrap 计算 CI")
  ...
  ## 点估计
  icc_simple <- calc_icc_simple(CL_ind_data[, -1])
  icc_CL_2_1 <- icc_simple$icc_2_1
  icc_CL_2_k <- icc_simple$icc_2_k
  
  ## Bootstrap 95% CI
  set.seed(12345)
  n_boot <- 1000
  icc_boot_2_1 <- numeric(n_boot)
  icc_boot_2_k <- numeric(n_boot)
  
  for (b in 1:n_boot) {
    idx <- sample(1:nrow(CL_ind_data), replace = TRUE)
    boot_data <- CL_ind_data[idx, -1]
    boot_icc <- calc_icc_simple(boot_data)
    icc_boot_2_1[b] <- boot_icc$icc_2_1
    icc_boot_2_k[b] <- boot_icc$icc_2_k
  }
  
  icc_CL_2_1_lo <- quantile(icc_boot_2_1, 0.025, na.rm = TRUE)
  icc_CL_2_1_hi <- quantile(icc_boot_2_1, 0.975, na.rm = TRUE)
  icc_CL_2_k_lo <- quantile(icc_boot_2_k, 0.025, na.rm = TRUE)
  icc_CL_2_k_hi <- quantile(icc_boot_2_k, 0.975, na.rm = TRUE)
  
  icc_CL_method <- "simplified + Bootstrap CI"
  
  cat("\n=== CL_ind ICC 结果（简化方法 + Bootstrap 95% CI）===\n")
  cat(sprintf("  ICC(2,1) = %.4f [95%% CI: %.4f - %.4f]\n", 
              icc_CL_2_1, icc_CL_2_1_lo, icc_CL_2_1_hi))
  cat(sprintf("  ICC(2,k) = %.4f [95%% CI: %.4f - %.4f]\n", 
              icc_CL_2_k, icc_CL_2_k_lo, icc_CL_2_k_hi))
...
## CI 解释
cat("\n  【CI 解释】\n")
if (icc_CL_2_1_lo >= 0.50) {
  cat("    ✅ 95% CI 下限 ≥ 0.50，一致性至少达到 Moderate 水平（稳健结论）\n")
} else if (icc_CL_2_1_lo >= 0.25) {
  cat("    ⚠️  95% CI 下限在 0.25-0.50 之间，一致性可能仅为 Fair-Moderate\n")
} else {
  cat("    ❌ 95% CI 下限 < 0.25，一致性证据不足\n")
}
...
CL_icc = list(
  icc_2_1    = icc_CL_2_1,
  icc_2_1_lo = icc_CL_2_1_lo,
  icc_2_1_hi = icc_CL_2_1_hi,
  icc_2_k    = icc_CL_2_k,
  icc_2_k_lo = icc_CL_2_k_lo,
  icc_2_k_hi = icc_CL_2_k_hi,
...
CL_summary_csv <- tibble(
  metric = c("CV_pop (%)", "CV_ind median (%)", "CV_ind IQR",
             "pct_CV_reduced (%)", 
             "ICC(2,1)", "ICC(2,1) 95% CI",  # <- 新增
             "ICC(2,k)", "ICC(2,k) 95% CI",  # <- 新增
  ...
  value  = c(
    ...
    sprintf("[%.4f, %.4f]", icc_CL_2_1_lo, icc_CL_2_1_hi),  # <- 新增
    sprintf("%.4f", icc_CL_2_k),
    sprintf("[%.4f, %.4f]", icc_CL_2_k_lo, icc_CL_2_k_hi),  # <- 新增
...
cat("\n【E.2 ICC】\n")
cat(sprintf("  ICC(2,1) = %.4f [95%% CI: %.4f - %.4f] (%s)\n", 
            icc_CL_2_1, icc_CL_2_1_lo, icc_CL_2_1_hi, interpret_icc(icc_CL_2_1)))
cat(sprintf("  ICC(2,k) = %.4f [95%% CI: %.4f - %.4f]\n", 
            icc_CL_2_k, icc_CL_2_k_lo, icc_CL_2_k_hi))
cat(sprintf("  计算方法: %s\n", icc_CL_method))
